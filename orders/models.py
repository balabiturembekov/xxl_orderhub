...
class Factory(models.Model):
    ...
    contact_person = models.CharField(
        max_length=100, blank=True, verbose_name="Контактное лицо"
    )
...
class Order(models.Model):
    ...
    status = models.CharField(
        max_length=20, choices=STATUS_CHOICES, default="uploaded", verbose_name="Статус"
    )
    uploaded_at = models.DateTimeField(auto_now_add=True, verbose_name="Дата загрузки")
    sent_at = models.DateTimeField(blank=True, null=True, verbose_name="Дата отправки")
    invoice_received_at = models.DateTimeField(
        blank=True, null=True, verbose_name="Дата получения инвойса"
    )
    ...
    reminder_count = models.PositiveIntegerField(
        default=0, verbose_name="Количество напоминаний"
    )

    # Напоминания фабрикам
    last_factory_reminder_sent = models.DateTimeField(
        blank=True,
        null=True,
        verbose_name="Последнее напоминание фабрике",
        help_text="Дата последнего напоминания фабрике о необходимости загрузить инвойс",
    )
    factory_reminder_count = models.PositiveIntegerField(
        default=0, verbose_name="Количество напоминаний фабрике"
    )

    # Отмена клиентом
    cancelled_by_client = models.BooleanField(
        default=False, verbose_name="Отменен клиентом"
    )
    cancelled_by_client_at = models.DateTimeField(
        blank=True, null=True, verbose_name="Дата отмены клиентом"
    )
    cancelled_by_client_comment = models.TextField(
        blank=True, verbose_name="Комментарий при отмене клиентом"
    )
    ...
    e_factura_turkey = models.BooleanField(
        default=False, verbose_name="E-Factura Turkey"
    )
    ...
    def is_turkish_factory(self):
        ...
        return (
            self.factory and self.factory.country and self.factory.country.code == "TR"
        )
    ...
    class Meta:
        ...
        indexes = [
            ...
            models.Index(
                fields=["cancelled_by_client_at"], name="order_cancelled_at_idx"
            ),
            models.Index(
                fields=["cancelled_by_client", "cancelled_by_client_at"],
                name="order_cancelled_comp_idx",
            ),
            ...
            models.Index(
                fields=["status", "uploaded_at"], name="order_status_upld_idx"
            ),
    ...
    def needs_reminder(self):
        ...
        if self.status == "uploaded" and self.days_since_upload >= 7:
            ...
            self.status == "sent" and self.days_since_sent and self.days_since_sent >= 7
        ):
            return True
        return False
    ...
    def mark_as_sent(self):
        ...
        if self.status != "uploaded":
            raise ValueError(
                f'Нельзя отметить как отправленный заказ со статусом {self.status}. Ожидается статус "uploaded".'
            )
        self.status = "sent"
    ...
    def mark_invoice_received(self, invoice_file):
        ...
        if self.status != "sent":
            raise ValueError(
                f'Нельзя отметить получение инвойса для заказа со статусом {self.status}. Ожидается статус "sent".'
            )
    ...
    def mark_as_completed(self):
        ...
        if self.status != "invoice_received":
            raise ValueError(
                f'Нельзя завершить заказ со статусом {self.status}. Ожидается статус "invoice_received".'
            )
    ...
    def clean(self) -> None:
        ...
        if (
            self.excel_file
            and hasattr(self.excel_file, "name")
            and self.excel_file.name
        ):
            validate_safe_filename(self.excel_file.name)
        if (
            self.invoice_file
            and hasattr(self.invoice_file, "name")
            and self.invoice_file.name
        ):
        ...
        if self.cancelled_by_client:
            ...
            if (
                self.cancelled_by_client_comment
                and len(self.cancelled_by_client_comment) > 2000
            ):
                from django.core.exceptions import ValidationError
...
class NotificationTemplate(models.Model):
    ...
    template_type = models.CharField(
        max_length=20, choices=TEMPLATE_TYPES, unique=True, verbose_name="Тип шаблона"
    )
...
class OrderConfirmation(models.Model):
    ...
    requested_by = models.ForeignKey(
        User, on_delete=models.CASCADE, verbose_name="Запросил"
    )
    ...
    confirmation_data = models.JSONField(
        default=dict, verbose_name="Данные подтверждения"
    )
    comments = models.TextField(blank=True, verbose_name="Комментарии")
    ...
    confirmed_at = models.DateTimeField(
        null=True, blank=True, verbose_name="Подтверждено"
    )
    expires_at = models.DateTimeField(
        verbose_name="Истекает", help_text="Автоматически устанавливается при создании"
    )
    ...
    class Meta:
        ...
        indexes = [
            models.Index(
                fields=["status", "expires_at"], name="conf_status_expires_idx"
            ),
        ...
        if not self.expires_at:
            ...
            if self.action == "send_order":
                self.expires_at = timezone.now() + timedelta(
                    hours=TimeConstants.CONFIRMATION_EXPIRATION_HOURS_SEND
                )  # 3 дня для отправки
            elif self.action == "upload_invoice":
                self.expires_at = timezone.now() + timedelta(
                    hours=TimeConstants.CONFIRMATION_EXPIRATION_HOURS_INVOICE
                )  # 2 дня для инвойса
            elif self.action in ["complete_order", "cancel_order", "delete_order"]:
                self.expires_at = timezone.now() + timedelta(
                    hours=TimeConstants.CONFIRMATION_EXPIRATION_HOURS
                )  # 1 день для критических
            else:
                self.expires_at = timezone.now() + timedelta(
                    hours=TimeConstants.CONFIRMATION_EXPIRATION_HOURS
                )  # По умолчанию 1 день
    ...
    def confirm(self, user, comments=""):
        ...
        if self.is_expired():
            ...
            if self.status == "pending":
                OrderConfirmation.objects.filter(id=self.id, status="pending").update(
                    status="expired"
                )
        ...
        updated = OrderConfirmation.objects.filter(
            id=self.id, status="pending", expires_at__gt=timezone.now()
        ).update(
            status="confirmed",
            confirmed_by=user,
...
class OrderAuditLog(models.Model):
    ...
    action = models.CharField(
        max_length=20, choices=ACTION_TYPES, verbose_name="Действие"
    )
...
class EmailTemplate(models.Model):
    ...
    language = models.CharField(
        max_length=5, choices=LANGUAGE_CHOICES, default="ru", verbose_name="Язык"
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, blank=True, verbose_name="Создал"
    )
    created_at = models.DateTimeField(auto_now_add=True, verbose_name="Дата создания")
    updated_at = models.DateTimeField(auto_now=True, verbose_name="Дата обновления")
...
class Invoice(models.Model):
    ...
    order = models.OneToOneField(
        Order, on_delete=models.CASCADE, verbose_name="Заказ", related_name="invoice"
    )
    ...
    total_paid = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, verbose_name="Сумма всех платежей"
    )
    remaining_amount = models.DecimalField(
        max_digits=12, decimal_places=2, default=0, verbose_name="Остаток к доплате"
    )
    status = models.CharField(
        max_length=20,
    ...
    def total_cbm(self):
        ...
        if (
            not self.pk
            or not hasattr(self, "order")
            or not self.order
            or not self.order.pk
        ):
            return 0
        ...
        try:
            total = (
                self.order.cbm_records.aggregate(total=Sum("cbm_value"))["total"] or 0
            )
            return total
        except Exception:
            return 0
    ...
    def clean(self):
        ...
        if self.balance is not None and self.balance <= 0:
            raise ValidationError({"balance": "Сумма инвойса должна быть больше нуля."})
    ...
    def save(self, *args, **kwargs):
        ...
        balance_decimal = (
            Decimal(str(self.balance)) if self.balance is not None else Decimal("0")
        )
        ...
        if is_new:
            ...
            if in_atomic:
                last_payment = (
                    self.payments.select_for_update().order_by("-created_at").first()
                )
            else:
            ...
            if last_payment:
                if last_payment.payment_type == "deposit":
                    ...
                    if self.total_paid >= balance_decimal:
                        ...
                        self.status = (
                            "partial"  # Финальный платеж, но сумма недостаточна
                        )
                else:
...
class InvoicePayment(models.Model):
    ...
    amount = models.DecimalField(
        max_digits=12, decimal_places=2, verbose_name="Сумма платежа"
    )
    ...
    def __str__(self):
        ...
        return (
            f"Платеж {self.amount} от {self.payment_date} по инвойсу {invoice_number}"
        )
...
class OrderCBM(models.Model):
    ...
    date = models.DateField(verbose_name="Дата", help_text="Дата добавления CBM")
    notes = models.TextField(
        blank=True,
        verbose_name="Комментарии",
    ...
    def __str__(self):
        ...
        if self.order and self.order.title:
            order_title = (
                self.order.title[:50] + "..."
                if len(self.order.title) > 50
                else self.order.title
            )
        else:
...
class Shipment(models.Model):
    ...
    def total_invoice_cbm(self):
        ...
        for order in self.orders.all():
            ...
            order_cbm = order.cbm_records.aggregate(total=Sum("cbm_value"))[
                "total"
            ] or Decimal("0")
            total += order_cbm
...
class EFacturaBasket(models.Model):
    ...
    month = models.PositiveIntegerField(
        verbose_name="Месяц", help_text="Номер месяца (1-12)"
    )
    year = models.PositiveIntegerField(
        verbose_name="Год", help_text="Год (например, 2025)"
    )
    basket_date = models.DateField(
        verbose_name="Дата корзины", help_text="Дата, связанная с корзиной"
    )
    created_by = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
    ...
    def get_or_create_for_month(cls, year, month, user=None):
        ...
        if not isinstance(month, int) or month < 1 or month > 12:
            raise ValidationError(
                f"Месяц должен быть числом от 1 до 12, получено: {month}"
            )

        if not isinstance(year, int) or year < 2000 or year > 2100:
            raise ValidationError(
                f"Год должен быть разумным числом (2000-2100), получено: {year}"
            )
        ...
        try:
            basket, created = cls.objects.get_or_create(
                month=month,
                year=year,
                defaults={"name": name, "basket_date": basket_date, "created_by": user},
            )
...
class EFacturaFile(models.Model):
    ...
    upload_date = models.DateField(
        verbose_name="Дата загрузки", help_text="Дата загрузки файла"
    )
    created_by = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.basket_id:
            ...
            if self.basket and self.upload_date:
                if (
                    self.basket.year != self.upload_date.year
                    or self.basket.month != self.upload_date.month
                ):
